...
if(isset($formato)){
	...
	while($monta1 = mysqli_fetch_array($sql1)){
		...
		while($montaAno = mysqli_fetch_array($sqlAno)){
			$ukey_client = $montaAno["ukey_client"];
			$cliente = $montaAno["cliente"];
	...
	while($monta1C = mysqli_fetch_array($sql1C)){
		...
		while($montaAnoC = mysqli_fetch_array($sqlAnoC)){
			...
			$ukey_clientC = $montaAnoC["ukey_client"];
			$clienteC = $montaAnoC["cliente"];
			...
			SELECT
				...
				FROM(
					...
					FROM mp_pedidos
					WHERE ukey_client = '".$ukey_clientC."'
					)dados
	...
	<div style="float:left;width:500px; padding-left:20px;">
		<h4>RESUMO DO FATURAMENTO POR STATUS E CLIENTES</h4>
		'.date('d/m/Y H:m:s').'
	</div>
	';
...
$name = 'FATURAMENTO POR ETAPAS E CLIENTES - '.date('d/m/Y').'.pdf';
