...
if(isset($formato)){
	...
	if($formato == "excel"){
		...
		$arquivo = 'faturamento_por_etapa_e_veiculo.xls';
		...
		<html lang ="pt-br">
			<head>
			<meta charset="utf-8">
			<title>Faturmento por Etapa e Veículo</title>
			</head>
			<body>
			<table border="1">
				<tbody>
	...
	while($monta1 = mysqli_fetch_array($sql1)){
		...
		while($montaAno = mysqli_fetch_array($sqlAno)){
			$ukey_vehicles = $montaAno["ukey_vehicles"];
			$veiculo = $montaAno["veiculo"];
			...
			SELECT
				...
				FROM(
					SELECT 
					   ...
					   valor_unit 
					FROM mp_pedidos
					WHERE ukey_vehicles = '".$ukey_vehicles."'
					)dados
	...
	while($monta1C = mysqli_fetch_array($sql1C)){
		...
		while($montaAnoC = mysqli_fetch_array($sqlAnoC)){
			...
			$ukey_vehiclesC = $montaAnoC["ukey_vehicles"];
			$veiculoC = $montaAnoC["veiculo"];
			...
			SELECT
				...
				FROM(
					...
					FROM mp_pedidos
					WHERE ukey_vehicles = '".$ukey_vehiclesC."'
					)dados
	...
	<div style="float:left;width:500px; padding-left:20px;">
		<h4>RESUMO DO FATURAMENTO POR STATUS E VEÍCULO</h4>
		'.date('d/m/Y H:m:s').'
	</div>
	';
...
if($formato == "excel"){
	...
	$name = 'FATURAMENTO POR ETAPAS E VEICULOS - '.date('d/m/Y').'.pdf';
